...
* Edit config.json
* Rerun _prepare.py_ . This will not touch the existing probeset, but will update all other meta information in basedata.json
* (Do *not* rerun _measure.py_ , measurements have already been created, right?)
* Sometimes you want to rerun _get-ips.py_ but typically not. For instance if you have updated OpenIPMap geoloc for some IPs you need to rerun this step. This needs a better solution, because this process is painfully slow.
